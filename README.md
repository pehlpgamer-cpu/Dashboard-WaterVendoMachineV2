...
Edit `src/services/firebaseConfig.ts` with your Firebase project credentials:
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /waterLogs/{document=**} {
      ...
      allow write: if request.auth.token.email.endsWith('@esp32.local') || 
                      request.auth.uid == 'SERVICE_ACCOUNT_UID';
    }
  }
}
```
...
│   ├── exportService.ts     # CSV/PDF/Excel
...
interface WaterLog {
  ...
  amount: number          // 3 or 5 (price in dollars)
...
### No data showing?
1. Check Firebase credentials in `firebaseConfig.ts`
...
store.averagePrice           // String ($X.XX)
...
exportService.exportToExcel(logs, filename)
